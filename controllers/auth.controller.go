...
import (
	...
	"github.com/dondonudon/authentication-authorization/utils"
...
func (ac *AuthController) ForgotPassword(ctx *gin.Context) {
	...
	user.PasswordResetAt = time.Now().Add(time.Minute * 10)
	ac.DB.Save(&user)
...
func (ac *AuthController) RefreshAccessToken(ctx *gin.Context) {
	...
	if !user.LoggedIn {
		ctx.AbortWithStatusJSON(http.StatusForbidden, gin.H{"status": "fail", "message": "User is not logged in"})
		return
	}
